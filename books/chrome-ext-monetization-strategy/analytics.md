...
**訂正**: 初版はここに「各段階の脱落率の目安（S-Hub実測）」として5段階の率（install→session_start 40〜60% ほか）を載せていました。**削除します。**

理由は2つあります。ひとつは、私たちが持っているのはイベントの件数だけで、「1人のユーザーがどこで落ちたか」を追える形になっていなかったこと。脱落率は本来 per-user の量ですが、件数の比からは出せません。もうひとつは、最終段（決済画面への到達）のイベントが、本書を出した時点では1件も記録されていなかったことです。

**段階の切り方そのものは使えます**（install → 起動 → Aha Moment → ペイウォール表示 → アップグレード開始 → 完了）。どこが自分の拡張のボトルネックかは、この区切りで**自分の数字を見て**判断してください。私たちの目安は、載せられるものがありません。
...
上の定義でリテンションを見るための GA4 探索レポートは組めます。ただし本書では、S-Hub の拡張別リテンションの数値を出しません。**この原稿を書いた時点で、私たちはそれを測れていませんでした。** リテンション用のテーブルは空のままで、拡張ごとの7日/14日/30日の残存率を出せる観測が存在しませんでした。

一般論として言えるのは、「毎日使う道具」と「必要なときだけ使う道具」ではリテンション曲線の形が違う、というところまでです。どちらが自分の拡張かは、上の手順で自分のデータを見て判断してください。私たちの数字を借りても、あなたの拡張の答えにはなりません。
...
- ファネルは件数でなく per-user で追えて初めて「脱落率」になる。計装を設計する時点で user 単位の追跡可否を決めておく
