...
18拡張の場合:
  ...
  同一ユーザーのLTV = 1本あたりのLTV × その人が使う拡張の数
```

**訂正**: 初版はここに「LTV = $31.92（月額×8ヶ月）」「S-Hub実測: 有料転換ユーザーの平均インストール拡張数 = 2.3本」「LTV = 約 $73/ユーザー」と書いていました。**$31.92 と $73 は削除します**（平均継続月数8ヶ月を出せる観測がなく、その掛け算の結果も支えられません）。「平均2.3本」は、**値そのものが誤っているとは限りませんが、「実測」として提示できる出所を示せません。** 実測と称したことを撤回します。ネットワーク効果の**構造**（1人のユーザーが複数の拡張を使えば LTV が積み上がる）は、この章の設計の前提として変わりません。
...
ReadMark のポップアップ設定画面の末尾に「S-Hubの他の拡張」セクションを追加しました。

**訂正**: 初版はここに「S-Hubの実測データ」として月間 2,400 表示・187 クリック（CTR 7.8%）・インストール率 41%・月間追加インストール 約77件 を載せていました。**削除します。** クリック数 187 は、私たちのクロスプロモが全期間に受け取ったクリックの総数を超えています。実際のクリック率は 1〜2% 台でした。7.8% という数字を出せる観測はありません。

クロスプロモは「効く/効かない」の前に**桁**を見てください。表示が月に数千、クリック率が数%なら、得られるのは月に数十クリックです。**それが何インストールになるかは、私たちは計測していません。** CAC がゼロであること（自分の拡張の中の枠なので）だけは、構造として言えます。
...
離脱防止としての位置づけです。（初版は ①〜⑤ の各配置に「CTR実測」の数値を載せていましたが、出所を示せないため削除しました。）

### ② オンボーディング完了画面

「セットアップ完了！」の画面末尾に「同じ開発者の他の拡張」を表示。最初の感動が高いタイミングで信頼を活かす。

Aha Moment 直後の感情的な高まりを活かす位置です。
...
レビューを書いてくれた高満足度ユーザーへのリーチです。

### ⑤ アンインストール防止モーダル

「本当に削除しますか？」モーダルで「こんな拡張も試しましたか？」を表示。ただし押しつけがましいと感じさせるリスクがあります。表示するかは慎重に判断。
...
- クロスプロモは獲得コストゼロ（自分の拡張の中の枠なので）。1人のユーザーが複数の拡張を使えば LTV は積み上がる
- 配置は「満足度が高いタイミング」（Review prompt 後・オンボーディング完了画面）を狙う。どこが効くかは自分のデータで測る
